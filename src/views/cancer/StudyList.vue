<template>
  <div>
    <a-modal v-model="visibleDialog" title="Study" @ok="addOrUpdate">
    ...
    >
      ...
      <span slot="action" slot-scope="text, record">
        ...
        <!-- <a-divider type="vertical" /> -->
        <!-- <a href="javascript:;" @click="updateInput(record.id)">编辑</a> -->
        <a-divider type="vertical" />
...
export default {
  ...
  methods: {
    ...
    add() {
      StudyAPi.add(this.form).then((resp) => {
    ...
    update() {
      StudyAPi.update(this.form.id, this.form).then((resp) => {
        this.$notification["success"]({
          message: "添加" + resp.data.message,
        });
        this.visibleDialog = false;
    ...
    createTSVFile() {
      StudyAPi.createTSVFile().then((res) => {
        var blob = res.data;
    ...
    initTSV(isEmpty) {
      StudyAPi.init({ isEmpty: isEmpty }).then((resp) => {
        // console.log(resp)
        this.loadData();
        this.$notification["success"]({
