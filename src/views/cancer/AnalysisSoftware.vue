<template>
  <div>
    <a-modal v-model="visibleDialog" title="AnalysisSoftware" @ok="addOrUpdate">
...
export default {
  ...
  methods: {
    ...
    add() {
      AnalysisSoftwareApi.add(this.form).then((resp) => {
    ...
    update() {
      AnalysisSoftwareApi.update(this.form.id, this.form).then((resp) => {
        this.$notification["success"]({
          message: "添加" + resp.data.message,
        });
        this.visibleDialog = false;
    ...
    createTSVFile() {
      AnalysisSoftwareApi.createTSVFile().then((res) => {
        var blob = res.data;
    ...
    initTSV(isEmpty) {
      AnalysisSoftwareApi.init({ isEmpty: isEmpty }).then((resp) => {
        // console.log(resp)
        this.loadData();
        this.$notification["success"]({
