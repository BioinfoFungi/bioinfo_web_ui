<template>
  <div>
    <a-modal v-model="visibleDialog" title="DataOrign" @ok="addOrUpdate">
...
export default {
  ...
  methods: {
    ...
    add() {
      DataOriginApi.add(this.form).then((resp) => {
    ...
    update() {
      DataOriginApi.update(this.form.id, this.form).then((resp) => {
        this.$notification["success"]({
          message: "添加" + resp.data.message,
        });
        this.visibleDialog = false;
